...
public class E_FixWriting extends Mission{
	...
	public E_FixWriting(MissionType mt2, String name, String korean, int clear, Location... locs) {
		...
		static boolean[][] connected = { {false,false,false,false}, {false,false,false,false}, {false,false,false,false} };									
			...
			for(int i=0;i<3;i++) {
				...
				for (int slot = 0; slot<54; slot++) {//gui인벤토리										
					...
					if (y==0||y==2||y==3||y==5) {									
						...
						if(x == 8) Util.Stack(gui.get(i), slot, Material.BLACK_STAINED_GLASS_PANE, 1, " ");//검정색표시								
						else if(x == 0) Util.Stack(gui.get(i), slot, Material.YELLOW_STAINED_GLASS_PANE, 1, " ");//노랑 표시								
						else if(x == 1 || x == 2) fillWire(gui.get(i), slot,wirecolorArray[i][yToidx(y)],x, i);//왼쪽 와이어 채우기(랜덤)								
						else if(x == 7) fillWire(gui.get(i), slot,yToidx(y), i); //오른쪽 와이어 채우기(고정)								
						else if(x == 6) ; //오른쪽 와이어 빈공간								
						else Util.Stack(gui.get(i), slot, Material.WHITE_STAINED_GLASS_PANE, 1, " "); //배경								
					}									
					else Util.Stack(gui.get(i), slot, Material.WHITE_STAINED_GLASS_PANE, 1, " "); //배경									
				}
			}
		...
		public void checkConnection(int code, int slot) {											
			new BukkitRunnable(){										
			public void run(){										
				if(slot%9==6&&slot<54&&slot/9!=1&&slot/9!=4) {									
					int idx  = yToidx(slot/9);								
					Util.debugMessage(slot+"슬롯 연결 확인");								
					if(!(gui.get(code).getItem(slot)==null) &&gui.get(code).getItem(slot).getType() == WIRECOLORARRAY[code][idx] )	{						
						...
						for (int i=0; i<4 ; i++)if (connected[code][i]==false) {							
	...
	public void onDrag(InventoryDragEvent e) {
			...
			String title = e.getView().getTitle();
			if(!title.contains("FixWiring")) return;
			int code = Integer.parseInt(title.replace("FixWriting", ""));
